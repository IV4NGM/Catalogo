...
async function getApiData() {
  ...
  for (let movie of movies.results) {
    ...
    for (let genreNumber of movieGenres) {
      movieGenresNameArray.push(
        genresID.genres.filter((e) => e.id == genreNumber)[0].name
      );
    }
...
function upgradeSilderData() {
  document.querySelector("#selected-score").textContent =
...
function applyFilters() {
  ...
  moviesArrayDisplay = moviesArray.filter((element) => {
    if (
      ...
      !element.title
        .toLocaleLowerCase()
        .includes(inputTitle.toLocaleLowerCase().trim())
    ) {
      return false;
    }
    if (inputLanguage !== "0") {
...
function displayMovies(moviesToDisplay) {
  ...
  if (moviesToDisplay.length == 0) {
    ...
    text1.textContent =
      "Ups, parece que no hay películas que coincidan con tus criterios de búsqueda.";
